...
func TestUpgradeSelfFunctions(t *testing.T) {
	...
	updated, ver, err := UpgradeSelf(ctx, "1.0.0")
	if err != nil {
		t.Logf("UpgradeSelf live check returned error (acceptable in offline CI): %v", err)
	} else {
		if updated {
			t.Errorf("expected updated=false when on latest 1.0.0, got true")
		}
		if ver == "" {
	...
	updated, ver, err = UpgradeSelfToPath(ctx, "1.0.0", destPath)
	if err != nil {
	...
	// 3. Test UpgradeSelfToPath with older version (0.0.1 < 1.0.0) -> downloads and upgrades!
	...
	if err != nil {
		...
		if ver != "1.0.0" {
			t.Errorf("expected upgraded version '1.0.0', got %q", ver)
		}
